...
func TestReElection2A(t *testing.T) {
	...
	t.Log("if the leader disconnects, a new one should be elected")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the new leader.
	t.Log("if the old leader rejoins, that shouldn't disturb the new leader.")
	cfg.connect(leader1)
...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	// cfg.rafts[leader1].becomeFollower()
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	// cfg.rafts[leader2].becomeFollower()
...
func TestBackup2B(t *testing.T) {
	...
	cfg.rafts[(leader1+0)%servers].becomeFollower()
